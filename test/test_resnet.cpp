...
std::shared_ptr<Tensor> PreProcessImage(const cv::Mat &image) {
    assert(!image.empty());
    // 调整输入大小
    cv::Mat resize_image;
    ...
    uint32_t input_c = 3;
    std::shared_ptr<Tensor> input = std::make_shared<Tensor>(input_c, input_h, input_w);
...
TEST(test_network, resnet1) {
    ...
    for (uint32_t i = 0; i < batch_size; ++i) {
        ...
        // 图像预处理
        std::shared_ptr<Tensor> input = PreProcessImage(image);
    ...
    std::vector<std::shared_ptr<Tensor>> outputs_softmax(batch_size);
    softmax_layer.forward(outputs, outputs_softmax);
    assert(outputs_softmax.size() == batch_size);

    for (int i = 0; i < outputs_softmax.size(); ++i) {
        const std::shared_ptr<Tensor> &output_tensor = outputs_softmax.at(i);
